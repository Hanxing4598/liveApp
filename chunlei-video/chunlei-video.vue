<template>
	<view>
		<video :src="src" :loop="loop" :show-center-play-btn="false" :controls="controls" :show-play-btn="false" :style="{ height: height }"
		 @waiting="waiting" :enable-progress-gesture="false" :objectFit="objectFit" :id="`video_${src}`" ref="`video_${src}`"
		 class="video" @timeupdate="timeupdate" @ended="ended">
		...
		<view class="progressBar-box">
			...
			<view :style="{
				'height':equipment.safeAreaInsets.bottom+20+'px'
			}"></view>
		</view>
	</view>
</template>
...
<script>
	export default {
		...
		created() {
			this.equipment = uni.getSystemInfoSync()
			this.equipment.safeAreaInsets = uni.getSystemInfoSync().safeAreaInsets ? uni.getSystemInfoSync().safeAreaInsets : {
				bottom: 0
			}
		},
		mounted() {
